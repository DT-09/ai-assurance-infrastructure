...
AAI_CORS_ORIGINS=os.getenv('AAI_CORS_ORIGINS','*')
SCIM_TOKEN=os.getenv('AAI_SCIM_TOKEN',''); KMS_PROVIDER=os.getenv('AAI_KMS_PROVIDER','hmac')
PROTOCOL_VERSION='1.0'; ENGINE_VERSION='15.0.0'


class Settings:
    """Runtime settings adapter used by the production middleware layer."""
    def __init__(self) -> None:
        self.environment = ENVIRONMENT
        self.cors_origins = [x.strip() for x in AAI_CORS_ORIGINS.split(',') if x.strip()] or ['*']
        self.trusted_hosts = [x.strip() for x in os.getenv('AAI_TRUSTED_HOSTS', '*').split(',') if x.strip()] or ['*']

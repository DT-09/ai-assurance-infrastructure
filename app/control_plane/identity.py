...
class IdentityStore:
    ...
    def create_key(self, organization_id: str, name: str = "default") -> dict:
        ...
        with self._connect() as db:
            db.execute(
                ...
                (key_id, organization_id, name, salt, key_hash, created_at, revoked_at)
                VALUES (?, ?, ?, ?, ?, ?, NULL)
                """,
    ...
    def authenticate(self, raw_key: str) -> Optional[str]:
        ...
        with self._connect() as db:
            rows = db.execute(
                "SELECT organization_id, salt, key_hash FROM api_keys WHERE revoked_at IS NULL"
            ).fetchall()
        for row in rows:
    ...
    def list_keys(self, organization_id: str) -> list[dict]:
        with self._connect() as db:
            rows = db.execute(
                """
                SELECT key_id, organization_id, name, created_at, revoked_at

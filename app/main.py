...
from .assurance.protocol import AssuranceProtocol
from .billing import BillingStore, StripeBilling
...
app.state.billing_store = BillingStore(config.billing_db)
app.state.stripe_billing = StripeBilling(app.state.billing_store)
...
def protocol_site():
    return FileResponse(STATIC_DIR / "protocol.html")
...
@app.get("/protocol.json", include_in_schema=False)
def protocol_manifest():
    return JSONResponse(AssuranceProtocol.manifest())


@app.get("/.well-known/ai-assurance-protocol.json", include_in_schema=False)
def protocol_discovery():
    return JSONResponse(AssuranceProtocol.manifest())


@app.get("/protocol/v1/schema.json", include_in_schema=False)
def protocol_schema():
    return FileResponse(BASE_DIR / "protocol" / "v1" / "schema.json", media_type="application/schema+json")


@app.post("/v1/verify/passport", include_in_schema=False)
def public_verify_passport(payload: dict):
    ...
    from .assurance.passport import verify_passport
    from .assurance.verification import verify_passport_evidence
    passport_valid = verify_passport(passport)
    evidence_result = verify_passport_evidence(passport, evidence)
    return {
...
@app.post("/v1/protocol/conformance", include_in_schema=False)
def protocol_conformance(payload: dict):
    return conformance_report(payload)
